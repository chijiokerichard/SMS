    ...
    <aside className="bg-black border-r border-r-[#8B909A] p-3.5 h-screen w-60 fixed">
        <header>
            ...
            <nav className="h-57.5 w-full flex flex-col gap-y-2">
                {Links.map((link)=>
               <Link key={link.name} href={link.path} className={`h-10 ${poppins.className}  text-[15px] flex items-center py-3.75 px-7.5 leading-5.5  ${link.path === pathName?"border-b-[#6734c7] bg-white border-b rounded-lg text-black":"text-[#e4e0e0] hover:bg-[#ece9f12b] hover:text-white rounded-lg"}  border-t-0 border-l-0 border-r-0`}>
